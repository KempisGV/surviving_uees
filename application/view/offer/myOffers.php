...
<html lang="en-US">
	<head>
		...
		<title>Directorio Compañías | SURVIVING UEES</title>
						...
						<div class="noo-main col-md-8">
							...
							<div class="masonry">
								<ul class="companies-overview masonry-container">
									<?php
										$locales = CompanyModel::getOffers();
									?>
									<li class="company-group masonry-item">
										<ul>
											<?php for($i = 0; $i < count($locales); $i++) {
												if ($locales[$i]->user_id == Session::get('user_id')) { ?>
											<li class="company-name"><a href="<?= Config::get('URL') . 'offer/offerdetail/' . $locales[$i]->offer_id; ?>"><?= $locales[$i]->title; ?></a></li>
											<?php }} ?>
										</ul>
									</li>
								</ul>
