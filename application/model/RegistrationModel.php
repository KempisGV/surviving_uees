...
class RegistrationModel {
    ...
    public static function registerNewRequest( $name, $lastname, $email, $document, $city, $phone) {
		...
		if (empty($name) OR empty($email) OR !filter_var($email, FILTER_VALIDATE_EMAIL) OR empty($document) OR !ValidationModel::validateDocument($document) OR empty($city) OR empty($phone) OR $email_exists OR $document_exists OR $email_exists_req OR $document_exists_req) {
			$email_flag = true;
			$document_flag = true;
		...
		$database = DatabaseFactory::getFactory()->getConnection();
        $sql = "INSERT INTO TB_REQUESTS ( name, lastname, email, document, city, phone) VALUES ( :name, :lastname, :email, :document, :city, :phone)";
        $query = $database->prepare($sql);
        $query->execute(array(
            ':name' => $name,
            ':lastname' => $lastname,
