...
class AdminModel {
	...
	public static function getRequestList() {
		...
		$sql = "SELECT request_id,  name, lastname, email, document, city, phone FROM TB_REQUESTS";
	...
	public static function getRequestDatabyId($request_id) {
		$database = DatabaseFactory::getFactory()->getConnection();
		$sql = "SELECT request_id, name, lastname, email, document, city, phone FROM TB_REQUESTS WHERE request_id = :request_id LIMIT 1";
		$query = $database->prepare($sql);
	...
	public static function approveUser($request_id) {
	    ...
	    copy('images/avatar/anonymous_big.png', 'images/avatar/' . $avatar);
				...
				$database = DatabaseFactory::getFactory()->getConnection();
				$sql = "INSERT INTO TB_USERS (name,lastname, document, phone, city, email, creation_timestamp, activation_hash, avatar,account_type) VALUES (:name,:lastname, :document, :phone, :city, :email, :creation_timestamp, :activation_hash, :avatar, 2)";
				$query = $database->prepare($sql);
				$query->execute(array(
	...
	public static function activateUser($user_id) {
		...
		//Se modifico el valor de active = 0 a active = 1
		$database = DatabaseFactory::getFactory()->getConnection();
		$sql = "UPDATE TB_USERS SET active = 1, activation_hash = :activation_hash WHERE user_id = :user_id LIMIT 1";
		$query = $database->prepare($sql);
		$query->execute(array(
			':user_id' => $user_id,
			':activation_hash' => $activation_hash
	...
	public static function updateRequest() {
		...
		if (empty($name) OR empty($email) OR !filter_var($email, FILTER_VALIDATE_EMAIL) OR empty($document) OR !ValidationModel::validateDocument($document) OR empty($city) OR empty($phone) OR $email_exists OR $document_exists) {
		...
		if ($request_info->name == $name AND $request_info->lastname == $lastname AND $request_info->email == $email AND $request_info->document == $document AND $request_info->city == $city AND $request_info->phone == $phone) {
			Session::add('feedback_positive', Text::get('notification/positive/request_updated'));
			return true;
		}

		$database = DatabaseFactory::getFactory()->getConnection();
        $sql = "UPDATE TB_REQUESTS SET  name = :name, lastname = :lastname, email = :email, document = :document, city = :city, phone = :phone WHERE request_id = :request_id LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(
	        ':request_id' => $request_id,
        	':name' => $name,
